...
$last_names = array(
  ...
  'Zimmerman');
	for($i = 0;$i < 20;$i++) {
		$fname = $first_names[rand(0, count($first_names))];
		$lname = $first_names[rand(0, count($last_names))];
		$phone = "";
		for($k = 0;$k < 10;$k++) {
			$phone .= mt_rand(0, 9);
		}
		$rank = mt_rand(1, 7);
		echo $fname . " " . $lname . " " . $phone . " " . $rank . "<br>";
		DBi::$conn->query("INSERT INTO users (fname, lname, phone, rank, date) VALUES ('" . $fname . "', '" . $lname . "', " . $phone . ", " . $rank . ", " . time() . ")");
	}
?>